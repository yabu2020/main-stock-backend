...
app.post('/registerassets', async (req, res) => {
  try {
    ...
    const savedAssets = await Promise.all(assets.map(async asset => {
      if (!asset.name || !asset.assetno || !asset.category || !asset.saleprice || !asset.purchaseprice) {
        throw new Error('Name, assetno, price, and category are required.');
      }
      ...
      // Here you would just save the asset without validating the price comparison
      return new AssetModel(asset).save();
    }));
    ...
    const assets = await AssetModel.aggregate([
      ...
      },
      {
        $group: {
          ...
          quantityType: { $first: "$quantityType" },
          status: { $first: "$status" }
        }
      },
      {
...
// });
app.post("/giveasset", async (req, res) => {
  const { assetId, userId, quantity, totalPrice } = req.body;

  console.log(`Received assetId: ${assetId}, userId: ${userId}, quantity: ${quantity}, totalPrice: ${totalPrice}`);

  if (!assetId || !userId || quantity <= 0 || totalPrice <= 0) {
    return res.status(400).json({ error: "Asset ID, User ID, quantity, and total price are required" });
  }
  ...
  try {
    ...
    const costPrice = asset.purchaseprice * quantity; // assuming `purchaseprice` is the cost price per unit
...
app.post('/reports', async (req, res) => {
  ...
  console.log(`Received request with startDate: ${start}, endDate: ${end}`);

  try {
    // Fetch assignments within the date range
    const assignments = await AssignmentModel.find({
      dateAssigned: { $gte: start, $lte: end }
    }).populate('asset').populate('user');

    // Fetch confirmed orders within the date range
    const confirmedOrders = await OrderModel.find({
      dateOrdered: { $gte: start, $lte: end },
      status: 'Confirmed'
    }).populate('asset').populate('userId');

    console.log(`Found assignments: ${assignments.length}`);
    console.log(`Found confirmed orders: ${confirmedOrders.length}`);

    if (assignments.length === 0 && confirmedOrders.length === 0) {
      return res.status(404).json({ error: 'No assignments or confirmed orders found for the given date range' });
    }

    // Calculate total sales and cost price for both assignments and confirmed orders
    const totalSalesFromAssignments = assignments.reduce((sum, assignment) => sum + (assignment.totalPrice || 0), 0);
    const costPriceFromAssignments = assignments.reduce((sum, assignment) => sum + (assignment.asset?.purchaseprice || 0) * (assignment.quantity || 0), 0);

    const totalSalesFromOrders = confirmedOrders.reduce((sum, order) => sum + (order.totalPrice || 0), 0);
    const costPriceFromOrders = confirmedOrders.reduce((sum, order) => sum + (order.asset?.purchaseprice || 0) * (order.quantity || 0), 0);

    // Total for both
    const totalSales = totalSalesFromAssignments + totalSalesFromOrders;
    const costPrice = costPriceFromAssignments + costPriceFromOrders;
    const profitOrLoss = totalSales - costPrice;

    // Combine assignments and orders data into reportData
    const reportData = [
      ...
      ...confirmedOrders.map(order => ({
        ...
        dateAssigned: order.dateOrdered,
        type: 'order'
      }))
    ];
...
app.get('/reports', async (req, res) => {
  try {
    ...
    console.log("Reports fetched:", reports); // Log data for inspection
    res.json(reports);
  } catch (error) {
